...
last_modified_at: 2023-07-09T02:40:00
---
...
2. Call Stack에 비동기함수가 들어오면 Event Loop에 의해 Task Queue / Event Queue로 옮겨지고, 여기서 비동기 처리를 진행한다
3. Task Queue / Event Queue에서 비동기 처리가 완료되고, Call Stack이 비게되면, 해당 task를 Call Stack으로 옮겨서 필요한 동기 처리를 완료한다
...
-> Async Programming을 위해 최근에 주로 사용하는 것이 async와 await 키워드이다
